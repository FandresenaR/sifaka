...
// PATCH /api/users/[id] - Met à jour le rôle d'un utilisateur (SUPER_ADMIN uniquement)
export async function PATCH(
  ...
  try {
    ...
    // Empêcher le super admin de se modifier lui-même
    ...
    if (!role || !['USER', 'EDITOR', 'ADMIN', 'SUPER_ADMIN'].includes(role)) {
      return NextResponse.json(
        { error: 'Rôle invalide' },
        { status: 400 }
      );
    ...
    // Empêcher de modifier le rôle du super admin principal (fandresenar6@gmail.com)
...
// DELETE /api/users/[id] - Supprime un utilisateur (SUPER_ADMIN uniquement)
export async function DELETE(
  ...
  try {
    ...
    // Empêcher de supprimer le super admin principal (fandresenar6@gmail.com)
